...
- [ ] **1.1 — WOW: drop a video, get its scene keyframes on-device.** Dropping (or picking) a
...
- [ ] **1.2 — WebCodecs decode in a Web Worker.** Demux (mp4box.js) and `VideoDecoder` run in a
...
- [ ] **1.3 — Live progress + scan-line feedback.** Decode/analysis shows real progress and the
...
- [ ] **1.4 — Design polish (contact sheet as hero).** The filmstrip contact sheet is the hero
...
- [ ] **2.1 — Sensitivity slider re-segments live.** A themed range slider changes the cut
...
- [ ] **2.2 — Stats readout.** Show duration, frames sampled, scenes found, and keyframes kept.
...
- [ ] **2.3 — Designed empty & loading states.** Before a file and during decode the stage is
  composed, not blank.
...
- [ ] **3.1 — Per-frame PNG export.** Each keyframe cell exports its full-resolution frame as a
...
- [ ] **3.2 — Download-all as a zip.** A single action bundles all kept keyframes into one zip.
...
- [ ] **3.3 — Design polish (export controls & success).** Export controls match the theme and a
...
- [ ] **4.1 — Error & unsupported handling.** Bad, DRM'd, or undecodable files and unsupported
...
- [ ] **4.2 — Responsive + a11y pass.** The page is composed and accessible at 390 / 768 / 1440.
...
- [ ] **4.3 — Static hosting & relative paths.** The app builds to a self-contained directory and
